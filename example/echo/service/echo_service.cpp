...
EchoService::EchoService(InetAddress const& addr, Epoll* ep) : addr_(addr), ep_(ep) {


}


void EchoService::StartUp() {
  Socket* socket = new Socket();
  InetAddress addr("127.0.0.1", 8888);
  socket->bind(addr);
  socket->listen();

  epoll_ = new xtc::Epoll();
  Channel* service_channel = new xtc::Channel(epoll, socket->GetFd());
...
void EchoService::on_recv_msg(xtc::Channel* client_channel) {
